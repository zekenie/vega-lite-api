...
import { api } from "../api";
export function generateDoc(schema, path, prefix) {
    ...
    jobs.push(write(`${path}/index.md`, docIndex(docIndexEntry, prefix)));
    return Promise.all(jobs);
}
function docIndex(generate, prefix) {
    // group into sections
    const sec = {};
...
function collectProperties(spec, schema) {
    ...
    for (prop in spec.pass) {
        if (spec.pass[prop] != null)
            props.push([prop, spec.pass[prop]]);
    }
    for (prop in spec.call) {
        if (spec.call[prop] != null)
            props.push([prop, spec.call[prop]]);
    }
    for (prop in schema) {
