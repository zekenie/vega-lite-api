...
export function generateMethod(schema, methodName, spec) {
    ...
    const className = capitalize(methodName), ext = spec.ext || {}, pass = spec.pass || {};
    ...
    for (let prop in schema) {
        if (hasOwnProperty(ext, prop))
            continue; // skip if extension defined
        if (hasOwnProperty(pass, prop))
            continue; // skip if pass defined
...
function generateProxy(emit, methodName, spec, opt) {
    const m = opt.call, a = opt.arg ? $(opt.arg) + ', ' : '';
...
function generateConstructor(emit, spec) {
    const arg = spec.arg, set = spec.set, type = spec.type;
    ...
    if (Array.isArray(arg)) {
        ...
        for (let i = 0, n = arg.length; i < n; ++i) {
            ...
            if (Array.isArray(_)) { // include a default value
                ...
                if (i !== 0)
                    error('Illegal argument definition.');
                emit.import(['get', 'set', 'merge']);
                if (t)
                    emit(`args = args.map(_ => ${typeSwitch(emit, t, '_')});`);
...
function generatePass(emit, method, opt) {
    ...
    if (!opt.self && !opt.init)
        emit.import('assign');
    emit(`${method}(...values) {`);
    if (opt.args)
        emit(`  values = values.slice(0, ${opt.args});`);
    if (opt.prop) {
        if (opt.init) {
...
function generateToObject(emit, spec) {
    const { key, nest } = spec, flag = Array.isArray(key);
    let obj = flag
