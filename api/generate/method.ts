...
import { Spec, Call, Field } from "../types";
...
function generateProxy(emit: Emit, methodName, spec: Spec, opt) {
  const m = opt.call,
    a = opt.arg ? $(opt.arg) + ", " : "";
...
function generateConstructor(emit: Emit, spec: Spec) {
  const arg = spec.arg,
    set = spec.set,
    type = spec.type;
  ...
  if (Array.isArray(arg)) {
    ...
    for (let i = 0, n = arg.length; i < n; ++i) {
      ...
      if (Array.isArray(_)) {
        ...
        emit.import("set");
        emit(
          `set(this, ${$(_[0])}, args[${i}] !== undefined ? args[${i}] : ${
            _[1]
          });`
        );
        ...
        emit(
          `set(this, ${$(_.slice(3))}, merge(0, get(this, ${$(
            _.slice(3)
          )}), args));`
        );
        break;
        ...
        // internal state, autogenerate id
        emit.import("id");
        emit(
          `this[${$(
            _.slice(1)
          )}] = args[${i}] !== undefined ? args[${i}] : id(${$(_.slice(2))});`
        );
      } else if (_.startsWith("_")) {
        // internal state
...
function generateExtension(emit: Emit, method: string, val) {
...
function generateMutations(obj: string, values: Record<string, string>): string[] {
  let code = [];
...
function generateNests(emit: Emit, nest: string[]) {
  if (nest) {
...
function generateCopy(emit: Emit, method: string, set: string[]) {
  emit.import("copy");
...
function typeSwitch(emit: Emit, types, value: string) {
  let code = "";
...
function generateProperty(emit: Emit, method: string, opt: Field) {
  ...
  if (mod) {
    if (multi) {
      ...
      emit(`value = value[0];`);
      if (type) emit(`value = ${typeSwitch(emit, type, "value")};`);
...
function generateMergedProperty(emit: Emit, method: string, opt: Field) {
  ...
  if (!pre) emit(`if (arguments.length) {`).indent();
  else emit(`values = [${$(pre)}].concat(values)`);

  if (type) emit(`values = values.map(v => ${typeSwitch(emit, type, "v")});`);
  emit(`const obj = copy(this);`);
...
function generateAccretiveObjectProperty(emit: Emit, method: string, opt: Field) {
  const { prop, pre, type, flag, set, nest } = opt;
  emit.import(["copy", "get", "merge", "set"]);
  ...
  if (!pre) emit(`if (values.length) {`).indent();
  else emit(`values = [${$(pre)}].concat(values)`);
...
function generateAccretiveArrayProperty(emit: Emit, method: string, opt: Field) {
  const { prop, pre, type, flag, set, nest } = opt;
  emit.import(["copy", "get", "merge", "set"]);
...
function generatePass(emit: Emit, method: string, opt: Call) {
...
function generateCall(emit: Emit, method: string, opt: Call) {
  emit.import(opt.call, opt.from || opt.call);
...
function generateToObject(emit: Emit, spec: Spec) {
  const { key, nest } = spec,
