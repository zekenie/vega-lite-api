    ...
    const spec = {
        ...
        ext: {
            ...
            if: {
                ...
                flag: 0,
                nest: ["test"],
                desc: "Perform a conditional encoding. If the provided condition (first argument) evaluates to true, apply the provided encoding (second argument).",
            },
            ...channelAggregate,
            ...channelTimeUnit,
        },
    };
    const fieldN = { key: "field", set: { type: N } }, fieldO = { key: "field", set: { type: O } }, fieldQ = { key: "field", set: { type: Q } };
    switch (type) {
        case "detail":
        case "tooltip":
            spec.type = {
...
export function unit(types) {
    const extMark = reduce(types, v => ({ arg: [':::mark'], pre: [{ type: v }] }), k => `mark${capitalize(k)}`);
    return {
